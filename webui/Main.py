...
def _open_task_path(task_path):
    ...
    if not normalized_path.startswith(tasks_root + os.sep) and not os.path.isdir(task_path):
...
def _open_task_video(video_file):
    ...
    if not normalized_file.startswith(tasks_root + os.sep) and not os.path.isfile(video_file):
        logger.warning(f"invalid task video path: {normalized_file}")
        return
    if not os.path.isfile(normalized_file) and not os.path.isfile(video_file):
